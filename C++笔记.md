...
##### 用法对比
两种写法效果完全一致，lambda更简洁：
```cpp
// lambda写法（直接嵌入）
find_if(v.begin(), v.end(), [](int val){return val>5;}) != v.end()

// 普通函数写法（需先定义函数）
find_if(v.begin(), v.end(), isGreaterThan5) != v.end()
```
...
| 部分         | 作用                          | 你的原例中对应内容          |
|--------------|-------------------------------|-----------------------------|
| `[捕获列表]`  | 控制是否使用外部变量（比如外面的x） | `[]`（空捕获：不用外部变量）|
| `(参数列表)`  | 接收的参数（和普通函数一样）  | `(int val)`：接收int类型参数 |
| `{函数体}`   | 核心逻辑（判断/计算等）       | `{return val>5;}`：判断是否大于5 |

##### 关键简化规则
1. 无参数时，`(参数列表)` 可省：`[]{return 3>5;}`（返回false）  
2. 函数体仅1句 `return` 时，返回值可省（编译器自动推断）：不用写 `-> bool`  
3. 函数体仅1句时，`{}` 可省（可选）：`[](int val) return val>5;`

##### 场景1：不使用外部变量（空捕获 `[]`）
逻辑只依赖容器元素（参数），不用外面的变量：
```cpp
// 找大于5的元素
...
// 找字符串长度>3的元素（容器存string）
[](const string& s) { return s.size() > 3; }
```

##### 场景2：使用外部变量（捕获 `[x]` 或 `[&x]`）
需要判断“大于外部变量x”（不是固定值），核心记2种捕获：
- `[x]`：值捕获（复制x，lambda内改x不影响外面）  
- `[&x]`：引用捕获（用x的地址，lambda内改x影响外面）
示例：找大于外部变量x的元素
```cpp
vector<int> v = {2,7,9};
int x = 6; // 外部变量
find_if(v.begin(), v.end(), [&x](int val) { 
    return val > x; // 用外部变量x
}) != v.end() ? cout << "找到大于" << x << "的值" << endl : cout << "没找到" << endl;
```

##### 场景3：排序（`sort` 配合lambda）
自定义排序规则，比普通函数简洁：
```cpp
vector<int> v = {3,1,4,2};

// 从小到大排序（return a < b）
sort(v.begin(), v.end(), [](int a, int b) { return a < b; }); // 结果：[1,2,3,4]

// 从大到小排序（仅改判断条件）
sort(v.begin(), v.end(), [](int a, int b) { return a > b; }); // 结果：[4,3,2,1]
```
##### 提示
 多语句需显式返回类型：函数体不止1句 `return` 时，要写 `-> 类型`：
```cpp
[](int val) -> bool { // 显式指定返回bool
